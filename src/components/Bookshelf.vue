<template>
  <div ref="bookshelfEl" class="sr-bookshelf" :class="{dragging:drag.file}" @drop="onDrop" @dragover="onDragOver" @dragleave="onDragLeave">
    ...
    <div class="sr-books">
      <Transition name="fade" mode="out-in">
        <div v-if="!displayItems.length" key="empty" class="sr-empty">
          <div>{{keyword?'未找到':'暂无内容'}}</div>
          <div v-if="!keyword" class="sr-empty-hint">拖拽文件到此处添加书籍</div>
        </div>
        <div v-else key="items" :class="`sr-${viewMode}`" :style="gridStyle">
          <template v-for="item in displayItems" :key="item.type+'-'+(item.type==='group'?item.data.id:item.data.url)">
            <!-- 分组 -->
            <div v-if="item.type==='group'" :class="['sr-card',viewMode,'sr-group',{dropping:drag.target===item.data.id}]" @click="selectGroup(item.data.id)" @contextmenu.prevent="showGroupMenu($event,item.data)" @dragover="onDragOver($event,item.data.id)" @dragleave="onDragLeave" @drop="onDrop($event,item.data.id)">
            ...
            <!-- 书籍 -->
            <div v-else :class="['sr-card',viewMode,{selected:selectedBooks.has(item.data.url),dragging:drag.book===item.data.url}]" draggable="true" @dragstart="drag.book=item.data.url" @dragend="drag.book=null;drag.target=null" @click="handleClick(item.data,$event)" @contextmenu.prevent="showContextMenu(item.data,$event)">
...
.sr-bookshelf{display:flex;flex-direction:column;height:100%;overflow:hidden;position:relative;&.dragging::after{content:'';position:absolute;inset:0;background:rgba(var(--b3-theme-primary-rgb),.05);border:2px dashed var(--b3-theme-primary);border-radius:8px;pointer-events:none;z-index:100}}
...
.sr-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;height:100%;font-size:14px;opacity:.5;.sr-empty-hint{font-size:12px;opacity:.7}}
