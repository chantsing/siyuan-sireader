...
const findProtyle = (range = getSelectionRange()): ProtyleLike | null => {
  ...
  const visit = (value: any): ProtyleLike | null => {
    ...
    if (Array.isArray(value)) { for (const item of value) { const hit = visit(item); if (hit) return hit } return null }
    if (seen.has(value)) return null
    seen.add(value)
    if (value?.protyle?.element === editor) return value.protyle
    return visit([value?.model?.editor, value?.model?.editors?.edit, value?.model?.editors?.unRefEdit, ...(Array.isArray(value?.model?.editors) ? value.model.editors : []), ...(Array.isArray(value?.children) ? value.children : []), ...(Array.isArray(value?.editors) ? value.editors : []), ...Object.values(value?.editors || {})])
  }
  const siyuan = (window as any).siyuan
  return visit([siyuan?.layout?.layout, siyuan?.dialogs, siyuan?.blockPanels])
}
const insertAtCursor = (content: string) => {
  const protyle = findProtyle(), instance = protyle?.getInstance?.()
...
const getCurrentDocId = async (blockId = '') => !blockId ? findActiveDocId() : (await api.getBlockByID(blockId))?.root_id || findActiveDocId()
...
const insertCurrent = async (text: string, settings: ReaderSettings) => {
  ...
  const blockId = getCurrentBlockId(), docId = await getCurrentDocId(blockId)
...
const getParentHPath = async (settings: ReaderSettings, parentID?: string) => {
  if (!parentID) return ''
  const notebook = getNotebookId(settings), path = (await api.getBlockByID(parentID).catch(() => null))?.path || ''
  return notebook && path ? await api.getHPathByPath(notebook, path).catch(() => '') : ''
}
const appendToNoteDoc = async (title: string, settings: ReaderSettings, text: string, key: string, parentID?: string) => {
  ...
  if (id) return api.appendBlock('markdown', text, id)
  const path = `${await getParentHPath(settings, parentID)}/${sanitize(title)}`.replace(/\/+/g, '/')
  const created = String(await api.createDocWithMd(notebook, path, text) || '')
